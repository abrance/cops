...
- 触发方式：`apps/**` 变更 push 到 `main` 时只部署受影响应用；`workflow_dispatch` 可手动指定或全量；每天定时全量重建以纠正漂移。

...
当前镜像地址由各应用目录下的 `.env` 文件直接声明，Compose 文件只负责把镜像名和版本 tag 拼接成最终引用。不要只修改文档示例而遗漏 `.env` 中的实际部署值。

## 2. 当前镜像清单

| 应用 | Compose 服务 | `.env` 变量 | 当前完整镜像 |
| --- | --- | --- | --- |
...
| `ptdoc` | `ptdoc` | `PTDOC_IMAGE` + `PTDOC_IMAGE_TAG` | `ghcr.chenby.cn/abrance/ptdoc:v1.0.8` |

配置来源：

- `apps/lems/.env`
- `apps/lems/compose.yaml`
- `apps/ptdoc/.env`
- `apps/ptdoc/compose.yaml`
...
- `*_IMAGE_TAG` 使用具体版本，例如 `v1.0.8`；不要使用 `latest`。
